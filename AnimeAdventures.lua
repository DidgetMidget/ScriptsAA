...
local function webhook()
	pcall(function()
		...
		local webHookElement = game:GetService("HttpService"):JSONEncode(data)

		local headers = {["content-type"] = "application/json"}
		request = http_request or request or HttpPost or syn.request or http.request
		local webHookRequest = {Url = url, Body = webHookElement, Method = "POST", Headers = headers}
...
function setUp()
	...
	credits:Label("-- Fixed some stuff --")
	credits:Label("Zero  ˞˞˞˞˞˞˞˞˞˞˞˞˞˞˞˞˞˞˞˞#0001")
...
coroutine.resume(coroutine.create(function()
	while task.wait() do
		if getgenv().autostart and getgenv().AutoFarm and getgenv().teleporting then
			if game.PlaceId == 8304191830 then
				while game.PlaceId == 8304191830 do
					for i, v in pairs(game:GetService("Workspace")["_LOBBIES"].Story:GetDescendants()) do
						if v.Name == "Owner" and v.Value == nil then
							getgenv().door = v.Parent.Name
